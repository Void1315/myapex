...
const CfgConfigContent = () => {
  const classes = useStyles({});
  const dispatch = useDispatch<AppDispatch>()
  const { apexRoot, steamCommand, originCommand } = useSelector((state: RootState) => state.cfgConfig)
  const handleClick = async () => {
    ipcRenderer.invoke('openApexDir').then((data) => {
      ...
      ipcRenderer.invoke('getGameStartCommand').then((data) => {
        ...
        dispatch({ type: 'cfgConfig/setState', payload: { steamCommand: steam, originCommand: origin } })
      })
      dispatch({ type: 'cfgConfig/setState', payload: { apexRoot: path, cfgData: _cfgData } })
    })
...
const ChildCfgFormItem = () => {
  const classes = useStyles({});
  const dispatch = useDispatch<AppDispatch>()
  const { cfgData } = useSelector((state: RootState) => state.cfgConfig)
  const changeCfgStatus = (e) => {
    ...
    ipcRenderer.invoke('changeChildCfgStatus', {
      ...
      dispatch({ type: 'cfgConfig/setState', payload: { cfgData: _cfgData } })
